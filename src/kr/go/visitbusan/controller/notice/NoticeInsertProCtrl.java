...
public class NoticeInsertProCtrl extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String savePath = "/dataNoti";		// 공지사항 첨부파일 저장 장소 정해야함
		...
		notice.setAttachment("./dataNoti/"+attFileName);
		
		System.out.println(nService.noticeIdGererator());
		System.out.println(notice.getNoticeTitle());
		System.out.println(notice.getNoticeContent());
		System.out.println(notice.getWrittenBy());
		...
		if(cnt==0){ //글쓰기 실패
			...
			response.sendRedirect("NoticeInsert.do");
